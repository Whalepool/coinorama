...
# Watcher class
class OKCoinLTCwkWatcher (coinwatcher.CoinWatcher):
    ...
    def fetchData ( self ):
        trades = '/api/v1/future_trades.do?symbol=ltc_usd&contract_type=this_week'
        if ( self.mostRecentTransactionID > 0 ):
            trades = trades + '&since=%d' % self.mostRecentTransactionID
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.okcoin.com', '/api/v1/future_depth.do?symbol=ltc_usd&contract_type=this_week', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'OKCoinLTCWeeklies', 'okcoinwkLTC', OKCoinLTCwkWatcher )
