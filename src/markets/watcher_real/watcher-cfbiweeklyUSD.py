...
# Watcher class
class CFBWUSDWatcher (coinwatcher.CoinWatcher) :
    ...
    def fetchData ( self ):
        trades = '/derivatives/api/tradingdata/trades?contract=F-XBT:USD-May16-W5&since=%d' %  int ( self.mostRecentTransactionID )
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.cryptofacilities.com', '/derivatives/api/cumulativebidask?tradeable=F-XBT:USD-May16-W5&unit=USD', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'CFBiWeekly', 'cfbiweeklyUSD', CFBWUSDWatcher )
