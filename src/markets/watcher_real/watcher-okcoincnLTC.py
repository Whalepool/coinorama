...
# Watcher class
class OKCoinltcCNYWatcher (coinwatcher.CoinWatcher):
    ...
    def fetchData ( self ):
        ...
        trades = '/api/v1/trades.do?symbol=ltc_cny'
        if ( self.mostRecentTransactionID > 0 ):
            trades = trades + '&since=%d' % self.mostRecentTransactionID
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.okcoin.cn', '/api/v1/depth.do?symbol=ltc_cny', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'OK-CoinLTCCNY', 'okcoincnLTC', OKCoinltcCNYWatcher )
