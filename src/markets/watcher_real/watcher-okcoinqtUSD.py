...
# Watcher class
class OKCoinUSDFQWatcher (coinwatcher.CoinWatcher):
    ...
    def fetchData ( self ):
        trades = '/api/v1/future_trades.do?symbol=btc_usd&contract_type=quarter'
        if ( self.mostRecentTransactionID > 0 ):
            trades = trades + '&since=%d' % self.mostRecentTransactionID
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'www.okcoin.com', '/api/v1/future_depth.do?symbol=btc_usd&contract_type=quarter', trades )
        return ed
...
if __name__ == "__main__":
    coinwatcher.main ( 'OKCoinQuarterlies', 'okcoinqtUSD', OKCoinUSDFQWatcher )
