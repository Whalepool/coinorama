...
class HuobiCNYWatcher (coinwatcher.CoinWatcher):
    ...
    def buildData ( self, book, trades, lag ):
        ...
        try:
            for t in trades['trades']:
                tid = int ( t['id'] )
                tvol = float ( t['amount'] )
                if ( ( tid > self.mostRecentTransactionID ) and ( t['ts'] > self.epoch ) ):
                    ed.volume += tvol
                    ed.nb_trades += 1
                if ( tid > mostRecent ):
                    mostRecent = tid
    ...
    def fetchData ( self ):
        ...
        ed = coinwatcher.CoinWatcher.fetchData ( self, httplib.HTTPSConnection, 'api.huobi.com', '/staticmarket/depth_btc_json.js', '/staticmarket/detail_btc_json.js' )
        return ed
